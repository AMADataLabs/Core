...
class VignettesTransformerTask(Task):
    def run(self):
        ...
        vignettes, codes, hcpcs_codes, administrative_codes = self._parse_data()
    ...
    def _parse_data(self):
        parsed_vignettes = pandas.read_csv(BytesIO(self._data[0]), delimiter='|', encoding='latin-1', skiprows=31,
                                           header=None, names=['cpt_code','long_descriptor','typical_patient',
                                                               'pre_service_info','intra_service_info',
                                                               'post_service_info', 'ruc_reviewed_date',
                                                               'survey_specialty'
                                                               ]
                                           )
        parsed_codes = pandas.read_excel(self._data[1]).rename(
            columns={'Concept Id': 'concept_id', 'CPT Code': 'cpt_code'}
            )
        parsed_hcpcs_codes = pandas.read_csv(BytesIO(self._data[2]), delimiter='\t').rename(
            columns={'Concept Id': 'concept_id','HCPCS II Code': 'cpt_code'}
            )
        parsed_administrative_codes = pandas.read_excel(self._data[3]).rename(
