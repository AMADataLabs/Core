...
#         env_from=[ETL_CONFIG],
#         secrets=[MINIO_SECRET],
#         env_vars=dict(TASK_CLASS='datalabs.etl.oneview.reference.transform.SpecialtyTransformerTask'),
#         do_xcom_push=False,
#         is_delete_operator_pod=True,
#         in_cluster=True,
...
#
    ...
    MIGRATE_DATABASE = KubernetesPodOperator(
        ...
        cmds=['./Script/migrate-database', 'create', 'OneView', '--message', 'Update tables', '--host',
              f'{DATABASE_SECRET.DATABASE_HOST}', '--port', f'{ETL_CONFIG.DATABASE_PORT}', '--name',
              f'{ETL_CONFIG.DATABASE_NAME}', '--password', f'{DATABASE_SECRET.DATABASE_PASSWORD}', '--username',
              f'{DATABASE_SECRET.DATABASE_USERNAME}'],
        do_xcom_push=False,
        is_delete_operator_pod=False,
        in_cluster=True,
