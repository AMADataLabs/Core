...
def get_RPV_result(phone, check_archive=True):
    ...
    if check_archive and phone in [p for p in df_archive['phone']]:
        ...
        print('Phone {} found in recently-tested archive. Re-using previous result.')
    ...
    # only make API call if phone is a 10-digit number or 10-character string of digits
        ...
        if response.status_code == 200:
            ...
            # pprint(response_json)  # debugging
            ...
            if status in kill_statuses:
                ...
                print(error_msg)
                print('SCRIPT ERROR for phone {}. Status: {}'.format(phone, status))
            ...
            # return the Series (can be appended to a larger result DataFrame) and the response JSON for logging
        ...
        # if there request was unsuccessful, record the response for that phone number and response
            ...
            print()
            print(phone, response_json['status'])
            print()
...
def get_RPV_results_df(phones, check_archive=True):
    ...
    print('Unique phone numbers to process: {}'.format(num_phones))
    ...
    if num_archived > 0:
        print('{} number' +
              ('s' if num_archived > 1 else '') +
              ' from list found in archived RPV results from past {} days.'.format(N_days_buffer))
...
def process_batch(df=None, check_archive=True, batch_del=False):
    ...
    phonecol = determine_phone_colname(df)
    print('Phone column: {}'.format(phonecol))

    df[phonecol] = df[phonecol].astype(str)

    phones = get_unique_phones_list(df)
    print('Number of unique phone numbers to process: {}'.format(len(phones)))

    print('Beginning RPV testing...')
    df_result = get_RPV_results_df(phones, check_archive)
    print('RPV results gathered! Joining results to inputs.')
    ...
    print('Creating output file at {}'.format(outfile_path))
    ...
    if batch_del:
        print('Making batch delete file.')
