    ...
    for name in cols_to_convert:
        if name in list(data_df.columns.values):
            if sum(data_df[name].notnull()) > 0:
                data_df[name] = data_df[name].apply(lambda x: round(int(x)) if ((x != None) and (~np.isnan(float(x)))) else x)
...
def create_new_addr_vars(orig_data):
    ...
    orig_data['addr_age_yrs'] = (pd.to_datetime(curr_time_df['time']) - pd.to_datetime(orig_data['ent_comm_begin_dt'])).apply(lambda x: x.days) / 365

    ###### Create new independent variable with representing how long doctor has been practicing
    orig_data['yop_yrs'] = (pd.to_datetime(curr_time_df['time']) - pd.to_datetime(orig_data['ppd_medschool_grad_year'])).apply(lambda x: x.days) / 365

    ###### Create new independent doctor age variable
    orig_data['doctor_age_yrs'] = (pd.to_datetime(curr_time_df['time']) - pd.to_datetime(orig_data['ppd_birth_year'])).apply(lambda x: x.days) / 365
