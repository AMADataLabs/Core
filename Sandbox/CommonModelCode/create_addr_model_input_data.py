...
def create_addr_key(data_df, addr_var, zip_var, st_num_var, addr_key_var):
    ...
    data_df[st_num_var] = data_df[addr_var].apply(lambda x: x[0:(x.find(' '))] if x[0:(x.find(' '))].isnumeric() else '')
...
def get_ent_counts(entity_addr_df, id_addr_var, all_addr_var, ent_id_var, addr_key_var):
    ...
    ent_id_addr_count_df = get_ent_addr_counts(entity_addr_df, [ent_id_var, addr_key_var], 
                                                    id_addr_var)
...
def join_ent_usg_count_data(ent_date_df, ent_id_addr_count_df, ent_all_addr_count_df, 
    ...
    ent_date_df = ent_date_df.drop(['usg_entity_id', 'usg_addr_key'], axis=1)

    ent_date_df = ent_date_df.merge(ent_all_addr_count_df,
                                    how='left',
                                    left_on='ent_addr_key',
                                    right_on='usg_addr_key')
    ent_date_df = ent_date_df.drop(['usg_addr_key'], axis=1)
...
def create_addr_entity_data(ent_comm_df, ent_comm_usg_df, post_addr_df, license_df, ent_key_df, date_df, date_var, date_me_var):
