...
def variable_breakdown(dataframe):
    ...
    for row in dataframe.itertuples():
        ...
        if 'IsMailable' in str(row.NoteDeccriptions):
            mail = 1
        if 'IsConnected' in str(row.NoteDeccriptions):
            connect = 1
        if 'IsPorted' in str(row.NoteDeccriptions):
            port = 1
        if 'NotValid' in str(row.NoteDeccriptions):
            novalid = 1
        if 'IsUnknownContact' in str(row.NoteDeccriptions):
            unknown = 1
        if 'IsPossibleDisconnected' in str(row.NoteDeccriptions):
            disconnected = 1
        if 'IsPossiblePortableVOIP' in str(row.NoteDeccriptions):
            voip = 1
        if 'IsWireless' in str(row.NoteDeccriptions):
            wireless = 1
