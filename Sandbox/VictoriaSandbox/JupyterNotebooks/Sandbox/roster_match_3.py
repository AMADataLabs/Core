...
def match_ppd_middle(matched_df):
    ...
    matched_df['MID_MATCH']=middle_match_listdd
...
def match_ppd_spec_fuzzy(matched_df):
    ...
    for row in matched_df.itertuples():
        keep = False
        if fuzz.ratio(row.DEPARTMENT.upper(), row.DESC_PRIM) > 80:
            keep = True
        elif row.DESC_PRIM in row.DEPARTMENT.upper():
            keep = True
        elif row.DEPARTMENT.upper() in row.DESC_PRIM:
            keep = True
        elif row.DESC_PRIM in row.TITLES.upper():
            keep = True
