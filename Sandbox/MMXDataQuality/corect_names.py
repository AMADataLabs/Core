...
count = 0
#data = data.apply(lambda x: x.str.strip() if x.name in ['LAST_NM', 'FIRST_NM', 'MIDDLE_NM'] else x)
#for col in data:
#    data[col] = data[col].astype(str).apply(lambda x: x.strip())
#print(data[data['LAST_NM']=='La'].index.tolist())
incorrect_df = pd.DataFrame(columns=['PARTY_ID','FIRST_NM','MIDDLE_NM','LAST_NM','ME_NUMBER'])
'''
print(data[(data['FIRST_NM'].isin(['Test1', 'Z-Test'])) | (data['LAST_NM'].isin(['Testperson', 'Zzz03'])) | ((data['FIRST_NM']=='Consumer') & (data['MIDDLE_NM']=='Affairs') & (data['LAST_NM']=='Tester')) | data['MIDDLE_NM']=='Nmn' ])
print(data.loc[(data['FIRST_NM']==data['LAST_NM']) & (data['MIDDLE_NM']!=None) & ~data['FIRST_NM'].isin(['Test1','Z-Test'])])
print(data.loc[(data['FIRST_NM']!=data['LAST_NM']) & (data['LAST_NM'].str.len()==1)])
print(data[data['LAST_NM'].isin(['Van', 'Von', 'Al', 'El', 'St']) & (data['FIRST_NM']!=data['LAST_NM'])]) #what if middle name is None
print(data.loc[(data['FIRST_NM']==data['LAST_NM']) & (data['MIDDLE_NM']!=None)])
#print(data.loc[((data['FIRST_NM']==data['MIDDLE_NM']) | (data['MIDDLE_NM']==data['LAST_NM']))& data['MIDDLE_NM']!='Van'] & ~data['FIRST_NM'].isin(['Test1','Z-Test']))
print(data.loc[(data['LAST_NM']=='De') & (data['FIRST_NM']=='La')])
print(data.loc[(data['LAST_NM']=='La') & (data['MIDDLE_NM']!=None) & (data['MIDDLE_NM']!='Tran')])
print(data.loc[(data['LAST_NM']=='Ben') & (data['FIRST_NM']=='Abda')])
print(data.loc[(data['LAST_NM']=='Del')])
print(data.loc[(data['LAST_NM']=='Delle')])
print(data.loc[(data['LAST_NM']=='San') & ((data['FIRST_NM']=='Jose') | (data['FIRST_NM']=='Juan'))])
'''
test_user = data.loc[(data['FIRST_NM'].isin(['Test1', 'Z-Test'])) | (data['LAST_NM'].isin(['Testperson', 'Zzz03'])) | ((data['FIRST_NM']=='Consumer') & (data['MIDDLE_NM']=='Affairs') & (data['LAST_NM']=='Tester')) | (data['MIDDLE_NM']=='Nmn') ]
...
'''
if data["LAST_NM"].astype("str").str.contains("La") :
    print(data['LAST_NM'])

if data['LAST_NM'].astype("str") == 'La' & data['FIRST_NM'].astype("str") =='De':
    count+=1
print(count)
'''
incorrect_df = incorrect_df.append([test_user, middle_to_last, last_nm_1, first_only, de_la, de, ben, Del, delle, san], ignore_index=True)
incorrect_df.info()
