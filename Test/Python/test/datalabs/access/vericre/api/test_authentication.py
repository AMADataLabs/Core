...
def test_token_response_error(passport_mixin, parameters, http_request_status_404):
    with mock.patch(
        ...
        "_request_eprofiles_access_token",
        return_value=http_request_status_404,
    ), pytest.raises(Exception) as except_info:
        passport_mixin._get_eprofiles_access_token(parameters)
...
def test_get_token(passport_mixin, http_request_status_200, parameters):
    with mock.patch(
        ...
        with mock.patch("datalabs.access.vericre.api.authentication.json.loads") as mock_json_loads:
            ...
            response = passport_mixin._get_eprofiles_access_token(parameters)
...
@pytest.fixture
def passport_mixin():
    passport_mixin = EProfilesAuthenticatingEndpointMixin
...
def parameters():
    ...
    parameters.client_secret = "client_secret"  # pragma: allowlist secret
