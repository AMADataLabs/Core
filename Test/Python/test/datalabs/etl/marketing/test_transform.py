...
def test_flatfile_updater_transformer_read_input_data(flatfile_updater_data):
    ...
    contacts, list_of_lists, flatfile, inputs = \
        read_input_merger_data(transformer,flatfile_updater_data)

    assert isinstance(contacts, pd.DataFrame)
    assert isinstance(list_of_lists, pd.DataFrame)
    assert isinstance(flatfile, pd.DataFrame)
    assert isinstance(inputs, pd.DataFrame)

    assert contacts.shape[0] == 7
    assert contacts.shape[1] == 2
    assert list_of_lists.shape[0] == 5
    assert list_of_lists.shape[1] == 6
...
def test_flatfile_updater_transformer_prune_flatfile_listkeys(flatfile_updater_data):
    transformer = FlatfileUpdaterTask({}, flatfile_updater_data)
    _, list_of_lists, flatfile,_ = \
        read_input_merger_data(transformer,flatfile_updater_data)

    flatfile = transformer._prune_flatfile_listkeys(flatfile, list_of_lists)
...
def test_flatfile_updater_transformer_assign_emppids_to_inputs(flatfile_updater_data):
    transformer = FlatfileUpdaterTask({}, flatfile_updater_data)
    _, list_of_lists, flatfile, inputs = \
        read_input_merger_data(transformer,flatfile_updater_data)

    flatfile = transformer._prune_flatfile_listkeys(flatfile, list_of_lists)

    matched_inputs, unmatched_inputs = \
        transformer._assign_emppids_to_inputs(inputs, flatfile)
...
def test_flatfile_updater_transformer_update_flatfile_listkeys(flatfile_updater_data):
    ...
    matched_inputs, unmatched_inputs = transformer._assign_emppids_to_inputs(inputs, flatfile)
...
def read_input_merger_data(transformer, input_merger_data):
    #print("*************", transformer._read_input_data(input_merger_data))
    return transformer._read_input_data(input_merger_data)
